...
export default function Home() {
  ...
  const [cinema, setCinema] = useState(false);     // para fade
  const [showScene, setShowScene] = useState(false); // para montar canvas
  const [planetIndex, setPlanetIndex] = useState(0);
  const [focusMode, setFocusMode] = useState(false);

  // 🎬 CUANDO SE DA CLICK EN COMENZAR
  const handleStart = () => {
    setCinema(true); // activa fade-out de pantalla inicial
    setTimeout(() => {
      setShowScene(true); // muestra canvas
      setStart(true);     // activa controles
    }, 1200);
  };
      ...
      content: (
        <div className="relative flex flex-col gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-semibold text-white">Daniela Coavas</h1>
              <p className="text-sm text-purple-100/90">
                Desarrolladora{" "}
                <span className="font-medium text-purple-200">Full Stack Junior</span>
              </p>
            </div>
          </div>
          ...
          <p className="text-sm leading-relaxed text-white/85">
            Me gusta construir experiencias digitales vivas, con animaciones suaves y
            interfaces mágicas. Estudio Ingeniería en la{" "}
            <span className="font-medium text-purple-200">Universidad EAN</span> y tengo la
            certificación{" "}
            <span className="font-medium text-yellow-200">AWS Cloud Practitioner</span>.
          </p>
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 rounded-xl border border-white/12 bg-white/4 px-4 py-3">
              ...
              <p className="text-sm text-purple-100">
                ...
                <span className="font-medium text-purple-200">BIT Institute</span>.
              </p>
            </div>
            ...
            <div className="flex items-start gap-3 rounded-xl border border-white/12 bg-white/4 px-4 py-3">
              ...
              <p className="text-sm text-purple-100">
                Core Team en{" "}
                <span className="font-medium text-purple-200">AWS Cloud Club EAN</span> y{" "}
                Women in Cloud.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-3 py-1 text-xs rounded-full bg-purple-500/20 text-purple-100 border border-purple-200/25">
                ☁️ Cloud Architecture
              </span>
              <span className="px-3 py-1 text-xs rounded-full bg-emerald-500/15 text-emerald-100 border border-emerald-300/20">
                🔧 Backend
              </span>
              <span className="px-3 py-1 text-xs rounded-full bg-pink-500/20 text-pink-100 border border-pink-200/40">
                🎨 UI/UX & Animations
              </span>
            </div>
          </div>
...
return (
  <div className="relative h-screen w-full overflow-hidden">
    ...
    {/* 🌌 ESTRELLAS 3D ANTES DE COMENZAR */}
    {!start && (
...
{!start && (
  <button
    ...
    className="
      ...
      px-12 py-4
      text-xl font-light text-white tracking-wide

      rounded-full
      bg-[#0d1128]/60      /* fondo suave para permitir glow interno */
      backdrop-blur-xl

      shadow-[0_0_45px_rgba(255,255,255,0.32)]
      hover:shadow-[0_0_65px_rgba(255,255,255,0.45)]

      before:content-[''] before:absolute
      before:inset-0 before:rounded-3xl
      before:bg-[radial-gradient(circle,_rgba(255,255,255,0.35)_0%,_rgba(255,255,255,0)_70%)]
      before:opacity-40
  ...
  >
    ...
    {/* --------------------------------------------- */}
    {/* 🔍 SOLO SE MUESTRA LA ESCENA CUANDO start = true */}
    {/* --------------------------------------------- */}
    {start && (

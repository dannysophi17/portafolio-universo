...
export default function SkillsCarousel({ translations: t, language }: SkillsCarouselProps) {
  ...
  const categories: SkillCategory[] = [
    ...
    },
    {
      ...
      skills: [{ name: 'Postman' }, { name: 'SCRUM' }, { name: 'Arduino' }],
      color: 'pink',
      gridCols: 'grid-cols-2 sm:grid-cols-3',
    },
    {
      ...
      skills: [
        {
          ...
          level: language === 'es' ? 'Nativo' : 'Native',
        },
  ...
  const techIcons = [
    ...
    },
    {
      ...
      svg: (
        <svg className="h-8 w-8" viewBox="0 0 24 24" fill="currentColor">
          <path d="M17.193 9.555c-1.264-5.58-4.252-7.414-4.573-8.115-.28-.394-.53-.954-.735-1.44-.036.495-.055.685-.523 1.184-.723.566-4.438 3.682-4.74 10.02-.282 5.912 4.27 9.435 4.888 9.884l.07.05A73.49 73.49 0 0111.91 24h.481c.114-1.032.284-2.056.51-3.07.417-.296 4.604-3.254 4.291-11.375zM12 14.58c-.288.14-.691.14-.979 0-.344-.165-2.875-2.062-2.875-5.58 0-3.519 2.531-5.415 2.875-5.58.288-.14.691-.14.979 0 .344.165 2.875 2.061 2.875 5.58 0 3.518-2.531 5.415-2.875 5.58z" />
        </svg>
      ),
    },
    {
  ...
  return (
    <div className="relative flex h-full flex-col items-center justify-end pt-24 pb-16 md:pt-32 md:pb-20">
      {/* Contenedor centrado verticalmente */}
      <div className="flex w-full max-w-7xl flex-col items-center justify-center space-y-4 px-2 sm:space-y-5 sm:px-3 md:space-y-6 md:px-6">
          ...
          {categories.map((category, index) => {
            ...
            return (
              <button
                ...
                } min-h-11 border text-xs backdrop-blur-sm md:text-sm`}
              >
                <div
                  className={`shrink-0 transition-colors ${activeTab === index ? 'text-white' : 'text-white/60 group-hover:text-white/90'}`}
                >
                  {category.icon}
                </div>
                <span
                  className={`xs:inline hidden font-medium transition-colors sm:inline ${activeTab === index ? 'text-white' : 'text-white/60 group-hover:text-white/90'}`}
                >
                  {category.title}
                </span>
              </button>
        ...
        <div className="w-full">
          <div key={activeTab} className="animate-fadeSlideIn w-full">
            {/* Category Header con gradiente de color */}
            <div
              className={`relative mb-3 flex items-center justify-center gap-2 rounded-xl bg-linear-to-r p-2.5 sm:mb-4 sm:gap-3 sm:p-3 ${colors.bg} border ${colors.border} backdrop-blur-sm`}
            >
              <div
                className={`h-7 w-7 rounded-lg sm:h-8 sm:w-8 md:h-10 md:w-10 ${colors.icon} flex shrink-0 items-center justify-center border backdrop-blur-sm`}
              >
                {currentCategory.icon}
              </div>
              <h3 className={`text-base font-bold sm:text-lg md:text-xl ${colors.text}`}>
            ...
            <div className={`grid ${currentCategory.gridCols} gap-2 md:gap-2.5`}>
              {currentCategory.skills.map((skill, index) => (
                <div
                  key={skill.name}
                  className="group relative animate-[slideUp_0.5s_ease-out_forwards] opacity-0"
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  {/* Card con tema del planeta */}
                  <div
                    className={`relative h-14 rounded-lg md:h-16 ${colors.card} backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${colors.glow} flex flex-col items-center justify-center p-2`}
                  >
                    <span
                      className={`relative text-xs font-semibold md:text-sm ${colors.text} text-center transition-colors group-hover:text-white`}
                    >
                      {skill.name}
                    </span>
                    {skill.level && (
                      <span className="relative mt-0.5 text-xs text-white/40 transition-colors group-hover:text-white/60">
                        {skill.level}
                      </span>
                    )}
                  </div>
